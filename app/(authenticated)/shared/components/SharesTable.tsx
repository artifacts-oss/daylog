...
export default function SharesTable({ shares }: { shares: any[] }) {
  const router = useRouter();
  const [isDeleting, setIsDeleting] = useState<string | null>(null);
  const [confirmDelete, setConfirmDelete] = useState<{ id: string, title: string } | null>(null);
  const [editingPassword, setEditingPassword] = useState<{ id: string, title: string } | null>(null);
    ...
    <div className="rounded-[20px] border border-border bg-card overflow-hidden shadow-sm">
      <Table>
        <TableHeader className="bg-muted/50">
          <TableRow>
            <TableHead className="w-[300px] font-bold text-[10px] uppercase tracking-[0.1em] text-muted-foreground/70">Entity</TableHead>
            <TableHead className="font-bold text-[10px] uppercase tracking-[0.1em] text-muted-foreground/70">Config</TableHead>
            <TableHead className="font-bold text-[10px] uppercase tracking-[0.1em] text-muted-foreground/70">Unique Readers (W/M/Total)</TableHead>
            <TableHead className="font-bold text-[10px] uppercase tracking-[0.1em] text-muted-foreground/70">Created</TableHead>
            <TableHead className="text-right font-bold text-[10px] uppercase tracking-[0.1em] text-muted-foreground/70">Actions</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {shares.length === 0 ? (
            <TableRow>
              <TableCell colSpan={5} className="h-48 text-center text-muted-foreground">
                <div className="flex flex-col items-center justify-center gap-2">
                  ...
                  <p className="text-[12px] opacity-60">Share a note or board to see it here.</p>
                </div>
              </TableCell>
            </TableRow>
          ) : (
              ...
              <TableRow key={share.id} className="hover:bg-muted/30 transition-colors group">
                <TableCell>
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col min-w-0">
                      <span className="font-bold text-[15px] text-foreground tracking-tight truncate max-w-[200px]">{share.title}</span>
                      <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/50">
                        {share.entityType === 'NOTE' ? 'Shared Note' : 'Public Board'}
                      </span>
                    </div>
                  </div>
                </TableCell>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    ...
                    {share.expiresAt && (
                      <span className={`text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded-md ${
                        ...
                        {new Date(share.expiresAt) < new Date() ? 'Expired' : 'Expires'}
                      </span>
                    )}
                    {share.oneTime && (
                    ...
                    {!share.password && !share.expiresAt && !share.oneTime && (
                ...
                <TableCell>
                  <div className="flex gap-1 items-baseline font-mono">
                    <span className="text-[14px] font-bold text-foreground">{share.metrics.weekly}</span>
                    <span className="text-[10px] text-muted-foreground/40">/</span>
                    <span className="text-[14px] font-medium text-muted-foreground">{share.metrics.monthly}</span>
                    <span className="text-[10px] text-muted-foreground/40">/</span>
                    <span className="text-[14px] font-bold text-primary">{share.metrics.total}</span>
                  </div>
                </TableCell>
                  ...
                  <div className="flex justify-end gap-1.5">
                    ...
                    {!(share.expiresAt && new Date(share.expiresAt) < new Date()) && (
                      <Button 
                        ...
                        className={`h-9 w-9 rounded-xl transition-all ${share.password ? 'text-fuchsia-500 hover:bg-fuchsia-500/10' : 'hover:bg-primary/10 hover:text-primary'}`}
                        title={share.password ? 'Update password' : 'Add password protection'}
                      >
                    ...
                    <Button 
                      variant="ghost" 
                      size="icon" 
                      onClick={() => setConfirmDelete({ id: share.id, title: share.title })} 
      ...
      <Dialog open={!!confirmDelete} onOpenChange={(open) => !open && setConfirmDelete(null)}>
        <DialogContent className="p-10 max-w-[480px] rounded-[32px] border border-border shadow-2xl">
          <DialogHeader className="mb-6">
            <span className="text-[12px] font-bold uppercase tracking-widest text-rose-500 mb-1 block">Security Verification</span>
          ...
          <div className="space-y-8">
            <p className="text-sm text-muted-foreground leading-relaxed antialiased font-medium">
              ...
              <strong className="text-rose-500 font-bold">{confirmDelete?.title}</strong>? This
              action will immediately stop all future access.
            </p>
            ...
            <div className="p-4 bg-[var(--color-accent-red)] rounded-[12px] border border-rose-500/20">
              <p className="text-[12px] text-rose-500 font-bold leading-normal flex gap-2">
                ...
                Warning: Revoking this link will permanently disable public access to this {confirmDelete?.title.includes('Board') ? 'board' : 'content'}.
              </p>
            </div>
          </div>
      ...
      <Dialog open={!!editingPassword} onOpenChange={(open) => !open && setEditingPassword(null)}>
        <DialogContent className="max-w-md rounded-[24px] border border-border mt-0">
          <DialogHeader>
            <DialogTitle className="text-xl font-bold tracking-tight">Update Protection</DialogTitle>
          </DialogHeader>
          <div className="py-6 space-y-4">
            <div className="space-y-1">
              <p className="text-[12px] font-bold uppercase tracking-wider text-muted-foreground/60">Target Link</p>
              <p className="font-bold text-foreground">{editingPassword?.title}</p>
            </div>
            <div className="space-y-1.5 pt-2">
              <p className="text-[12px] font-bold uppercase tracking-wider text-muted-foreground/60">New Password</p>
              <Input 
                 type="text" 
              ...
              />
              <p className="text-[11px] text-muted-foreground italic">Setting a new password will invalidate the previous one immediately.</p>
            </div>
          </div>
          <DialogFooter className="gap-2 sm:gap-0">
            <Button variant="ghost" onClick={() => setEditingPassword(null)} className="rounded-xl font-bold">Cancel</Button>
            <Button 
